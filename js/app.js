...
function leerDatosPlatillo(platillo){
    const infoPlatillo = {
        ...
        cantidad: 1,
        id: platillo.querySelector('img').getAttribute('data-id')
    }
...
function insertarCarrito(platillo) {
    ...
    row.innerHTML = `
       ...
       <td>
       <input type="number" class="contador" value="1" id="contador" data-id="${platillo.cantidad}" name="contador" min="1" max="100">
       </td>
       <td>${platillo.cantidad}</td>
       <td>
            <img  class="borrar-platillo" src="img/eliminar.png" alt="eliminarProducto" data-id="${platillo.id}">
       </td>
    `;
...
function eliminarPlatillo(e) {
    ...
    if(e.target.classList.contains('borrar-platillo')) {
        ...
        platilloId = platillo.querySelector('img').getAttribute('data-id');
    }
...
function leerLocalStorage() {
    ...
    platillosLS.forEach(function(platillo){
        ...
        row.innerHTML = `
            ...
            <td>${platillo.precio}</td>
            <td>
                <input type="number" class="contador" id="contador" value="1" 
                data-id="${platillo.cantidad}" name="contador" min="1" max="100">
            </td>
            <td>${platillo.cantidad}</td>
            <td>
